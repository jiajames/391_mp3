 ...
 *       print 8 hexadecimal igits, zero-padded on the left.
...
void copy_buffer_to_video(char *src){
	int i;
	for(i = 0; i < (NUM_ROWS*NUM_COLS);i++)
	{
			*(uint8_t *)(video_mem + (i << 1)) = src[i];
        	*(uint8_t *)(video_mem + (i << 1) + 1) = ATTRIB;
	}
	
}

void copy_video_to_buffer(char *dst){
	int i;
	for(i = 0; i < (NUM_ROWS*NUM_COLS);i++)
	{
		  dst[i] = *(uint8_t *)(video_mem + (i << 1)) ;
	}
}
...
void pressed_enter(){
	int i ;
	for(i = 0; i < (NUM_ROWS*NUM_COLS -80);i++)
	{
		  *(uint8_t *)(video_mem + (i << 1)) = *(uint8_t *)(video_mem + ((i+80) << 1));
        *(uint8_t *)(video_mem + (i << 1) + 1) = ATTRIB;

	}
	for(i = NUM_ROWS*NUM_COLS -80; i < NUM_ROWS*NUM_COLS; i++)
		  *(uint8_t *)(video_mem + (i << 1)) = ' ';
        *(uint8_t *)(video_mem + (i << 1) + 1) = ATTRIB;
...
#define VGA_CURSOR_ADDR 0x3D4
#define VGA_CURSOR_DATA 0x3D5
#define VGA_CURSOR_UPPER 0x0F
#define VGA_CURSOR_LOWER 0x0E

void update_cursor(int row, int col){
	unsigned short pos= (row*NUM_COLS) + col;

	outb (VGA_CURSOR_UPPER, VGA_CURSOR_ADDR);
	outb((unsigned char)(pos & 0xFF) , VGA_CURSOR_DATA);
	outb (VGA_CURSOR_LOWER, VGA_CURSOR_ADDR);
	outb((unsigned char)(pos>>8 & 0xFF) , VGA_CURSOR_DATA);
}
